        ...
        try {
            ...
            if (!upgradeHeader || upgradeHeader !== 'websocket') {
                ...
                if (ul == '/') {
                    ...
                    const fakeConfig = await cepz(userID, request.headers.get('Host'), sub, 'CF-Workers-SUB', RproxyIP, url, env);
                    return new Response(`${fakeConfig}`, {status: 200});
                } else if (url.pathname == `/${tiaoid}/edit` || ul == `/${userID}/edit`) {
                    return await KV(request, env);
                } else if (url.pathname == `/${tiaoid}` || ul == `/${userID}`) {
                    await sendMessage(`#获取订阅 ${FileName}`, request.headers.get('CF-Connecting-IP'), `UA: ${UA}</tg-spoiler>\n域名: ${url.hostname}\n<tg-spoiler>入口: ${url.pathname + url.search}</tg-spoiler>`);
                    const whisinfo = await cepz(userID, request.headers.get('Host'), sub, UA, RproxyIP, url, env);
                    const now = Date.now();
                    ...
                    if (userAgent && userAgent.includes('mozilla')) {
                        return new Response(`<div style="font-size:13px;">${whisinfo}</div>`, {
                            status: 200,
                            headers: {
    ...
    readableWebSocketStream.pipeTo(new WritableStream({
        async write(chunk, controller) {
            ...
            if (!banHosts.includes(addressRemote)) {
                ...
                handleTCPOutBound(remoteSocketWapper, addressType, addressRemote, portRemote, rawClientData, webSocket, whisRH, log);
            } else {
...
async function handleTCPOutBound(remoteSocket, addressType, addressRemote, portRemote, rawClientData, webSocket, whisRH, log,) {
...
function makeReadableWebSocketStream(webSocketServer, earlyDataHeader, log) {
    ...
    return new ReadableStream({
        ...
        start(controller) {
            ...
            // WebSocket 协议要求在每个方向上都要发送单独地关闭消息，以完全关闭 Socket
...
async function handleDNSQuery(udpChunk, webSocket, whisRH, log) {
    ...
    try {
        ...
        let whisH = whisRH; // 保存 维列斯 响应头部，用于后续发送
...
async function shuangsha(texts) {
    ...
    const onehex = oneshaarr.map(bits => bits.toString(16).padStart(2, '0')).join('');

    const twosha = await crypto.subtle.digest('MD5', bmq.encode(onehex.slice(7, 27)));
    const twoshaarr = Array.from(new Uint8Array(twosha));
    const twohex = twoshaarr.map(bits => bits.toString(16).padStart(2, '0')).join('');
...
async function dul(durl, murl) {
    ...
    // 添加自定义头部，包含 URL 信息
    //xxy.headers.set('X-Proxied-By', 'Cloudflare Worker');
    //xxy.headers.set('X-Original-URL', wurl);
...
function pzinfo(UUID, ymdz) {
    ...
    const WTR = `${xylx}://${UID}@${dz}:${dk}\u003f\u0065\u006e\u0063\u0072\u0079` + 'p' + `${atob('dGlvbj0=') + jmfs}\u0026\u0073\u0065\u0063\u0075\u0072\u0069\u0074\u0079\u003d${csaq[0]}&sni=${SNI}&fp=${zw}&type=${csXY}&host=${wzym}&path=${encodeURIComponent(ul)}#${encodeURIComponent(bm)}`;
...
async function cepz(userID, hostName, sub, UA, RproxyIP, _url, env) {
    ...
    if (userAgent.includes('mozilla') && !subParams.some(_searchParams => _url.searchParams.has(_searchParams))) {
        ...
        let dyq = '<br>';
        if (sub) {
            if (enableSocks) dyq += `CFCDN（访问方式）: Socks5<br>&nbsp;&nbsp;${newSocks5s.join('<br>&nbsp;&nbsp;')}<br>${socks5List}`;
            else if (proxyIP && proxyIP != '') dyq += `CFCDN（访问方式）: ProxyIP<br>&nbsp;&nbsp;${proxyIPs.join('<br>&nbsp;&nbsp;')}<br>`;
            else if (RproxyIP == 'true') dyq += `CFCDN（访问方式）: 自动获取ProxyIP<br>`;
            else dyq += `CFCDN（访问方式）: 无法访问, 需要您设置 proxyIP/PROXYIP ！！！<br>`
            ...
            if (addresses.length > 0) dyq += `ADD（TLS优选域名&IP）: <br>&nbsp;&nbsp;${addresses.join('<br>&nbsp;&nbsp;')}<br>`;
            if (addressesnotls.length > 0) dyq += `ADDNOTLS（noTLS优选域名&IP）: <br>&nbsp;&nbsp;${addressesnotls.join('<br>&nbsp;&nbsp;')}<br>`;
            if (addressesapi.length > 0) dyq += `ADDAPI（TLS优选域名&IP 的 API）: <br>&nbsp;&nbsp;${addressesapi.join('<br>&nbsp;&nbsp;')}<br>`;
            if (addressesnotlsapi.length > 0) dyq += `ADDNOTLSAPI（noTLS优选域名&IP 的 API）: <br>&nbsp;&nbsp;${addressesnotlsapi.join('<br>&nbsp;&nbsp;')}<br>`;
            if (addressescsv.length > 0) dyq += `ADDCSV（IPTest测速csv文件 限速 ${DLS} ）: <br>&nbsp;&nbsp;${addressescsv.join('<br>&nbsp;&nbsp;')}<br>`;
        }

        if (tiaoid && _url.pathname !== `/${tiaoid}`) dyq = '';
        else dyq += `<br>SUBAPI（订阅转换后端）: ${subProtocol}://${subConverter}<br>SUBCONFIG（订阅转换配置文件）: ${subConfig}`;
        const dtUID = (uuid != userID) ? `TOKEN: ${uuid}<br>UUIDNow: ${userID}<br>UUIDLow: ${userIDLow}<br>${userIDTime}TIME（动态UUID有效时间）: ${time01} 天<br>UPTIME（动态UUID更新时间）: ${time02} 时（北京时间）<br><br>` : `${userIDTime}`;
        return `
			################################################################<br>
        ...
        try {
            ...
            if ((!sub || sub == "") && isBase64 == true) {
                content = celocaldy(fakeHostName, fakeUserID, noTLS, newAddressesapi, newAddressescsv, newAddressesnotlsapi, newAddressesnotlscsv);
            } else {
                ...
                if (lines[0].split(',').length > 3) {
                    ...
                    for (let i = 1; i < lines.length; i++) {
                        ...
                        if (columns) {
                            ...
                            if (api[index].includes('proxyip=true')) proxyIPPool.push(`${columns}:${csdk}`);
                        }
                    }
                } else {
                    ...
                    if (api[index].includes('proxyip=true')) {
                        ...
                        proxyIPPool = proxyIPPool.concat((await shoushi(content)).map(item => {
...
function celocaldy(host, UUID, noTLS, newAddressesapi, newAddressescsv, newAddressesnotlsapi, newAddressesnotlscsv) {
    ...
    if (noTLS == 'true') {
        ...
        notlsresponseBody = uniqueAddressesnotls.map(address => {
            ...
            return `${xylx}://${UUID}@${address}:${port + atob('P2VuY3J5cHRpb249bm9uZSZzZWN1cml0eT0mdHlwZT13cyZob3N0PQ==') + wzym}&path=${encodeURIComponent(zzul)}#${encodeURIComponent(addressid + jdbz)}`;
    ...
    let base64Response = uniqueAddresses.map(address => {
        let port = "-1";
        let addressid = address;
        ...
        const xylx = atob(what);
        return `${xylx}://${UUID}@${address}:${port + atob('P2VuY3J5cHRpb249bm9uZSZzZWN1cml0eT10bHMmc25pPQ==') + wzym}&fp=random&type=ws&host=${wzym}&path=${encodeURIComponent(zzul)}#${encodeURIComponent(addressid + jdbz)}`;
    }).join('\n'); // 重新进行 Base64 编码
...
async function shoushi(contents) {
    ...
    // 然后将连续的多个逗号替换为单个逗号
    let recontent = contents.replace(/[	|"'\r\n]+/g, ',').replace(/,+/g, ',');

    // 删除开头和结尾的逗号（如果有的话）
    if (recontent.charAt(0) == ',') recontent = recontent.slice(1);
    if (recontent.charAt(recontent.length - 1) == ',') recontent = recontent.slice(0, recontent.length - 1);

    // 使用逗号分割字符串，得到地址数组
    return recontent.split(',');
}
...
function cetiaoid(my) {
    const sqpy = 8; // 北京时间相对于UTC的时区偏移+8小时
    const srq = new Date(2007, 6, 7, time02, 0, 0); // 固定起始日期为2007年7月7日的凌晨3点
    ...
    function ceID(jcstr) {
        ...
        return crypto.subtle.digest('SHA-256', shahc).then((sha) => {
            ...
            return `${slsha.slice(0, 8)}-${slsha.slice(8, 12)}-4${slsha.slice(12, 15)}-${(parseInt(slsha.slice(15, 17), 16) & 0x3f | 0x80).toString(16)}${slsha.slice(17, 19)}-${slsha.slice(19, 31)}`;
        });
    }

    const nowz = getnowz(); // 获取当前周数
    const etime = new Date(srq.getTime() + nowz * yzhm);

    // 生成两个 UUID
    const dnowUIDP = ceID(my + nowz);
    const prUIDP = ceID(my + (nowz - 1));

    // 格式化到期时间
    const endtimeUTC = new Date(etime.getTime() - sqpy * 60 * 60 * 1000); // UTC时间
    const endtimestr = `到期时间(UTC): ${endtimeUTC.toISOString().slice(0, 19).replace('T', ' ')} (UTC+8): ${etime.toISOString().slice(0, 19).replace('T', ' ')}\n`;

    return Promise.all([dnowUIDP, prUIDP, endtimestr]);
}
