    ...
    if (userAgent.includes('mozilla') && !subParams.some(_searchParams => _url.searchParams.has(_searchParams))) {
        ...
        try {
            ...
            if ((!sub || sub == "") && isBase64 == true) {
                content = await celocaldy(fakeHostName, fakeUserID, noTLS, newAddressesapi, newAddressescsv, newAddressesnotlsapi, newAddressesnotlscsv);
            } else {
...
function celocaldy(host, UUID, noTLS, newAddressesapi, newAddressescsv, newAddressesnotlsapi, newAddressesnotlscsv) {
    ...
    if (noTLS == 'true') {
        ...
        notlsresponseBody = uniqueAddressesnotls.map(address => {
            ...
            const whisL = `${xylx}://${UUID}@${address}:${port + atob('P2VuY3J5cHRpb249bm9uZSZzZWN1cml0eT0mdHlwZT13cyZob3N0PQ==') + wzym}&path=${encodeURIComponent(zzul)}#${encodeURIComponent(addressid + jdbz)}`;
    ...
    const responseBody = uniqueAddresses.map(address => {
        ...
        const xylx = atob(what);
        const whisL = `${xylx}://${UUID}@${address}:${port + atob('P2VuY3J5cHRpb249bm9uZSZzZWN1cml0eT10bHMmc25pPQ==') + wzym}&fp=random&type=ws&host=${wzym}&path=${encodeURIComponent(zzul)}#${encodeURIComponent(addressid + jdbz)}`;
...
function cetiaoid(my) {
    ...
    function ceID(jcstr) {
        ...
        return crypto.subtle.digest('SHA-256', shahc).then((sha) => {
            ...
            return `${slsha.substr(0, 8)}-${slsha.substr(8, 4)}-4${slsha.substr(13, 3)}-${(parseInt(slsha.substr(16, 2), 16) & 0x3f | 0x80).toString(16)}${slsha.substr(18, 2)}-${slsha.substr(20, 12)}`;
        });
    }
