    ...
    async fetch(request, env, ctx) {
        try {
            ...
            if (!upgradeHeader || upgradeHeader !== "websocket") {
                ...
                if (subConverter.includes("https://")) {
...
function celocaldy(
    ...
    let base64Response = uniqueAddresses
        .map((address) => {
            let port = "-1";
            let addressid = address;
            ...
            const xylx = atob(what);
            return `${xylx}://${UUID}@${address}:${port + atob("P2VuY3J5cHRpb249bm9uZSZzZWN1cml0eT10bHMmc25pPQ==") + wzym
...
function cetiaoid(my) {
    ...
    function ceID(jcstr) {
        ...
        return crypto.subtle.digest("SHA-256", shahc).then((sha) => {
            ...
            return `${slsha.slice(0, 8)}-${slsha.slice(8, 12)}-4${slsha.slice(
                12,
                15
            )}-${((parseInt(slsha.slice(15, 17), 16) & 0x3f) | 0x80).toString(
                16
            )}${slsha.slice(17, 19)}-${slsha.slice(19, 31)}`;
        });
    }
